...
public class RhinoModel<T extends Entity> extends HierarchicalModel<T> {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        PartDefinition torso = body.addOrReplaceChild("torso", CubeListBuilder.create()
                .texOffs(0, 0).addBox(-10.0F, -38.0F, -26.0F, 20.0F, 24.0F, 16.0F, new CubeDeformation(0.0F))
        ...
        // Skull and features (horns, ears, eyes) of the Rhino
        PartDefinition skull = head.addOrReplaceChild("skull", CubeListBuilder.create()
                .texOffs(69, 29).addBox(-6.0F, -9.0F, -12.0F, 12.0F, 20.0F, 12.0F, new CubeDeformation(0.0F))
        ...
        // Eyes and eyelids
        PartDefinition left_eye = skull.addOrReplaceChild("left_eye", CubeListBuilder.create()
                .texOffs(55, 46).addBox(-0.45F, -0.4F, -0.9F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F))
        ...
        PartDefinition right_eye = skull.addOrReplaceChild("right_eye", CubeListBuilder.create()
                .texOffs(55, 46).mirror().addBox(-0.55F, -0.4F, -0.9F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F)).mirror(false)
        ...
        PartDefinition tail = torso.addOrReplaceChild("tail", CubeListBuilder.create()
                .texOffs(16, 88).addBox(-1.5F, 0.0F, 0.0F, 3.0F, 0.0F, 13.0F, new CubeDeformation(0.0F))
    ...
    // Updates animation states, head rotation, and performs walking and idle animations
    @Override
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose); // Reset all parts to their default pose
        this.applyHeadRotation(netHeadYaw, headPitch, ageInTicks); // Apply head rotation based on the entity's inputs

        // Animate walk and idle states
    ...
    // Applies clamped rotation values to the head based on head pitch and yaw
    private void applyHeadRotation(float pNetHeadYaw, float pHeadPitch, float pAgeInTicks) {
        pNetHeadYaw = Mth.clamp(pNetHeadYaw, -30.0F, 30.0F); // Clamp yaw between -30° and 30°
        pHeadPitch = Mth.clamp(pHeadPitch, -25.0F, 45.0F);   // Clamp pitch between -25° and 45°

        this.head.yRot = pNetHeadYaw * ((float) Math.PI / 180F); // Convert yaw to radians
        this.head.xRot = pHeadPitch * ((float) Math.PI / 180F); // Convert pitch to radians
    }

    // Render the Rhino's model to the buffer with the given visual parameters
    @Override
