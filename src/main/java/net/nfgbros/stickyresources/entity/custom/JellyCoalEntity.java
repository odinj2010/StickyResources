...
public class JellyCoalEntity extends JellyEntity {

    // Time until the entity drops coal
    public int dropTime;
    ...
    public JellyCoalEntity(EntityType<? extends JellyEntity> pEntityType, Level pLevel) {
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.dropTime <= 0) {
            ...
            // Spawn coal item at the entity's location
            this.spawnAtLocation(ModItems.STICKY_COAL.get());
            // Trigger game event
