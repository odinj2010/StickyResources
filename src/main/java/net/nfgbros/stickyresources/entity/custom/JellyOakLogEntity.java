...
public class JellyOakLogEntity extends JellyEntity {
    ...
    // Time until the entity drops an oak log
    public int dropTime;
    ...
    public JellyOakLogEntity(EntityType<? extends JellyEntity> pEntityType, Level pLevel) {
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.dropTime <= 0) {
            ...
            // Spawn oak log block at the entity's location
    ...
    public boolean canMate(Animal otherAnimal) {
        // Check if both animals are JellyEntity and in love
        if (otherAnimal instanceof JellyEntity && this.isInLove() && otherAnimal.isInLove()) {
            return true;
        }
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        // Handle breeding logic based on parent types
        if (this.getType() == ModEntities.JELLY_OAK_LOG.get() && pOtherParent.getType() == ModEntities.JELLY.get()) {
            // Breed JellyOakLog with Jelly -> JellyOakLog
            return ModEntities.JELLY_OAK_LOG.get().create(pLevel);
        } else if (this.getType() == ModEntities.JELLY_OAK_LOG.get() && pOtherParent.getType() == ModEntities.JELLY_OAK_LOG.get()) {
            // Breed JellyOakLog with JellyOakLog -> JellyOakLog
            return ModEntities.JELLY_OAK_LOG.get().create(pLevel);
        }
        return null; // No offspring if other combinations
    }
